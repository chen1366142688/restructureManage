  ...
  <div style="padding-left:10px;">
    ...
    <Tabs type="card" :value="selectTabName" @on-click="selectTabs">
        <TabPane
              ...
              :label="grade.gradeName+'('+grade.startSchool+'级)'"
        ...
        <div v-if="gradeData.gradeId==grade.gradeId" v-for="gradeData,indexa in allData">
          ...
          <div style="margin-top:20px;" v-if="gradeData.stopList.dataList.length>0">
            <h2 class="run_title">已停用项目</h2>
            <div
              v-if="gradeData.stopList.dataList"
              v-for="examItem in gradeData.stopList.dataList"
              style="margin-top:10px;border: 1px solid rgba(191, 201, 206, 1);"
              :dis-hover="true"
            >
              <div v-if="gradeData.stopList.dataList">
                <div class="project-info-data">
                  <div class="pro-l">
                    <div class="test_project" style="width:100px;word-break:break-all;text-align: center">体能</div>
                  </div>
                    ...
                    <ul>
                      <li v-for="(selectedItem,sindex) in item.actionList" :key="sindex">
                        <div class="interview-item__name">
                          ...
                          <span v-if="selectedItem.enableStatus==1">{{selectedItem.classTestName}}</span>
                          <span v-else style="color:red">{{selectedItem.classTestName}}(已停用)</span>
                          <i
    ...
    <Modal
      ...
      >
        <Row style="margin-top: 10px;">
          <Col span="24">
            <span style="margin-left: 20px;">{{itemName}}与{{yearNameOther}}项目重复，是否继续选择？</span>
          </col>
          </Row>
        <div slot="footer">
            <Button type="text" size="large" @click="checkAllSure">确定</Button>
            <Button type="text" size="large" @click="cancel">取消</Button>
        </div>
      </Modal>
      <Modal
              v-model="noticeModal1"
              title="提醒"
              width="400px"
      >
        <Row style="margin-top: 10px;">
        ...
        <div slot="footer">
            <Button type="text" size="large" @click="checkGroupSure">确定</Button>
            <Button type="text" size="large" @click="cancel">取消</Button>
        </div>
      </Modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    checkAllSure() {
      var vm = this;
      vm.cancel();
      let indexa = vm.indexa;
      let indexb = vm.indexb;
      let classTestInfo = vm.classTestInfo;
          ...
          for (let x = 0; x < allData[indexa].selectList.length; x++) {
            if (
              ...
              let param = {
                ...
                enableStatus: classTestInfo.enableStatus,
    ...
    handleCheckAll(classTestInfo, indexa, indexb) {
      ...
      vm.indexa = indexa;
      vm.indexb = indexb;
      vm.classTestInfo = classTestInfo;
      vm.itemName = classTestInfo.classTestName;
      let tf = classTestInfo.checkAll;
      if (classTestInfo.disabled && !tf) {
        vm.noticeModal = true;
        return;
      }
      vm.checkAllSure();
    ...
    checkGroup(classTestInfo, indexa, indexb, indexc) {
      ...
      vm.itemName = classTestInfo.classTestName;
      let tf = classTestInfo.list[indexc].selectTf;
      if (classTestInfo.list[indexc].disabled && !tf) {
        vm.noticeModal1 = true;
        return;
      }
      vm.checkGroupSure();
    },
    ...
    selectTabs(name) {
      ...
      sessionStorage.setItem(
        "actionStaminaHisConfige-selectTabName",
        this.selectTabName
      );
    ...
    getAllGradeClassTestList() {
      ...
      this.$axios
        .get(
          "/v1/actionStamina/getAllGradeClassTestList?classTestType=" +
            vm.classTestType +
            "&yearSemester=" +
            vm.yearSemester
        )
    ...
    dealData() {
      ...
      for (let i = 0; i < vm.allDataDe.length; i++) {
        ...
        gradeInfo.stopList = {};
        gradeInfo.stopList.list = [];
        gradeInfo.stopList.items = [];
        gradeInfo.stopList.dataList = [];
        if (gradeInfo.list) {
          if (
          ...
          for (let q = 0; q < gradeInfo.list.length; q++) {
            ...
            if (classTestInfo.enableStatus == 1) {
              if (classTestInfo.classTestType == "2") {
                gradeInfo.actionList.list.push(classTestInfo);
                if (
                  gradeInfo.actionList.items.indexOf(classTestInfo.itemName) ==
                  -1
                ) {
              ...
              gradeInfo.stopList.list.push(classTestInfo);
              if (
                gradeInfo.stopList.items.indexOf(classTestInfo.itemName) == -1
              ) {
                gradeInfo.stopList.items.push(classTestInfo.itemName);
                let datas = {
                  itemName: classTestInfo.itemName,
                  list: [],
                };
                gradeInfo.stopList.dataList.push(datas);
              }
            }
          }
          ...
          //已停止
          for (let j = 0; j < gradeInfo.stopList.list.length; j++) {
            let classTestInfo = gradeInfo.stopList.list[j];

            // for (let w = 0; w < gradeInfo.stopList.dataList.length; w++) {
            //   if (
            //     classTestInfo.itemName ==
            //     gradeInfo.stopList.dataList[w].itemName
            //   ) {
            //     gradeInfo.stopList.dataList[w].list.push(classTestInfo);
            //   }
            // }
            classTestInfo.open = false;
            classTestInfo.disabled = false;
            ...
            for (let w = 0; w < gradeInfo.stopList.dataList.length; w++) {
              if (classTestInfo.itemName == gradeInfo.stopList.dataList[w].itemName ) {
                gradeInfo.stopList.dataList[w].list.push(classTestInfo);
              }
            }
          }
    ...
    baseSubmit() {
      ...
      this.$axios
        .get(
          "/v1/actionStamina/querySchoolReportedStatus?dataType=2&yearSemester=" +
            this.yearSemester
        )
        .then((res) => {
          if (res.data.code === 10000) {
            ...
            if (editStatus === "1") {
              this.$axios
                .post("v1/examtest/checkExamAndTestRelation", data)
                .then((res) => {
                  if (res.data.code == 10000) {
    ...
    save(list) {
      ...
      this.$axios
        .post("/v1/actionStamina/upsertStudentClassSelect", data)
        .then((res) => {
          if (res.data.code == 10000) {
            if (res.data.response.code == "1") {
    ...
    paramsCheck() {
      var vm = this;
      let yearSemester = Number(
        sessionStorage.getItem(
          "actionStaminaHisConfige-actionStaminaConfigeChoose-yearSemester"
        )
      );
      vm.yearSemester = yearSemester;
      if (vm.yearSemester % 2 == 1) {
        vm.yearName =
          (vm.yearSemester - 9 + "").substring(0, 4) + "学年第二学期";
        vm.yearNameOther =
          (vm.yearSemester - 9 + "").substring(0, 4) + "学年第一学期";
      } else {
        vm.yearName = (vm.yearSemester + "").substring(0, 4) + "学年第一学期";
        vm.yearNameOther =
          (vm.yearSemester + "").substring(0, 4) + "学年第二学期";
      }
      let gradeList = JSON.parse(
        sessionStorage.getItem("actionStaminaHisConfige-gradeList")
      );
      vm.gradeList = gradeList;
      ...
      if (
        ...
        vm.selectTabName = vm.gradeList[0].gradeName;
        vm.gradeId = vm.gradeList[0].gradeId;
      } else {
        vm.selectTabName = selectTabNameInfo;
        for (let i = 0; i < vm.gradeList.length; i++) {
          if (selectTabNameInfo == vm.gradeList[i].gradeName) {
            vm.gradeId = vm.gradeList[i].gradeId;
          }
        }
      }
      sessionStorage.setItem(
