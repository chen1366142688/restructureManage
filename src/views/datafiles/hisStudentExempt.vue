    ...
    return {
      ...
      normal: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const text = row.exemptId != null ? "免试" : "未免试";
            return h("div", text);
          },
        },
        {
            ...
            if (row.exemptTestName) {
              ...
              for (var i = 0; i < list.length; i++) {
                if (
                  ...
                  text +=
                    "<p style='line-height:30px;'><lable>" +
                    list[i] +
                    "</lable></p>";
                } else {
                  text +=
                    "<p style='line-height:30px;'>运动技能/体能 - <lable>" +
                    list[i] +
                    "</lable></p>";
                }
              }
  ...
  methods: {
    ...
    getStudentExamExemptList() {
      ...
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-pageNum",
        vm.pageNum
      );
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-pageSize",
        vm.pageSize
      );
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-grade",
        vm.grade
      );
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-studentClass",
        vm.studentClass
      );
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-testTypeId",
        vm.testTypeId
      );
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-studentName",
        vm.studentName
      );
      sessionStorage.setItem(
        "hisStudentExempt-getStudentInfoList-registerCode",
        vm.registerCode
      );

      this.$axios
        .post("/v1/datafiles/exempt/getHisStudentExamExemptList", data)
        .then((res) => {
          vm.loading = false;
    ...
    countStudentList(schoolId) {
      ...
      this.$axios
        .post("/v1/datafiles/exempt/countHisStudentExamExemptList", data)
        .then((res) => {
          if (res.data.code == 10000) {
    ...
    toDetail(studentId) {
      ...
      let ck = JSON.parse(sessionStorage.getItem("checkedna"));
      if (ck == "") {
        sessionStorage.setItem("checkednameList", "");
      }
      console.log(studentId);
      this.$router.push({ name: "hisStudentExemptDetail" });
    },
  ...
  mounted() {
    ...
    if (schoolType) {
      ...
      let yearSemester = sessionStorage.getItem(
        "hisStudentExempt-getStudentInfoList-yearSemester"
      );
      ...
      if (
        ...
        vm.pageNum = Number(
          sessionStorage.getItem("hisStudentExempt-getStudentInfoList-pageNum")
        );
        vm.pageSize = Number(
          sessionStorage.getItem("hisStudentExempt-getStudentInfoList-pageSize")
        );
