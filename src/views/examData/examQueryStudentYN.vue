...
export default {
  ...
  methods: {
    ...
    toDetail(studentInfo){
      ...
      this.$router.push({ name: "studentExamDetailYN" });
    },
    getExerciseStudentList(schoolId) {
     var vm = this;
      ...
      this.$axios
        .post("/v1/exam/getExamStudentList2", data)
        .then(res => {
          vm.loading = false;
          if (res.data.code == 10000) {
            let result = res.data.response;
              if(result != null && result.length > 0){
                      ...
                      for(let i=0;i<examList.length;i++){
                        if(examList[i].score !=null){
                          ...
                          if(examList[i].resultValue == null){
                              ...
                              examList[i].baseScore = examList[i].baseScore==null || examList[i].baseScore==""? "--":examList[i].baseScore;
                              examList[i].teacherScore = examList[i].teacherScore==null  || examList[i].teacherScore==""? "--":examList[i].teacherScore;
                ...
                if(skillCount>0){
                  result[j].skillTotalScore=skillTotalScore.toFixed(2)
                  // result[j].skillTotalScore=(skillTotalScore/skillCount).toFixed(2);
                }else{
                  result[j].skillTotalScore="--"
                }
                if(staminaCount>0){
                  result[j].staminaTotalScore=staminaTotalScore.toFixed(2)
                  // result[j].staminaTotalScore=(staminaTotalScore/staminaCount).toFixed(2);
                }else{
