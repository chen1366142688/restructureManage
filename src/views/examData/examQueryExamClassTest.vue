    ...
    return {
      ...
      resultColumns:[
        ...
        },
        {
          ...
          minWidth:100,
          render: (h, params) => {
            return h('div', params.row.excellentPercent+'%' + ' （' + params.row.excellentStudent + '人）')
          }
        },
        {
          title: '良好率（人数）',
          ...
          minWidth:100,
          render: (h, params) => {
            return h('div', params.row.goodPercent+'%' + ' （' + params.row.goodStudent + '人）')
          }
        },
        {
          title: '及格率（人数）',
          ...
          minWidth:100,
          render: (h, params) => {
            return h('div', params.row.passPercent+'%' + ' （' + params.row.passStudent + '人）')
          }
        },
        {
          title: '优良率（人数）',
          ...
          render: (h, params) => {
            ...
            return h('div', `${params.row.finishStudent > 0 ? (count / params.row.finishStudent * 100).toFixed(2) : 0.00}%（${count}人）`);
          }
        },
        {
          title: '合格率（人数）',
          ...
          render: (h, params) => {
            ...
            return h('div', params.row.allPassPercent+'%' + ' （' + count + '人）')
          }
        },
      ],
      resultData:[],
      classColumns:[
          ...
          render: (h, params) => {
            ...
            if(list.length>0){
              for(let i=0;i<list.length;i++){
                let data=list[i];
                text+= `<div style='width:100%;border-bottom:${i === list.length-1 ? 0 : 1}px #e8eaec solid;height:30px;line-height:30px;'>`
          ...
          render: (h, params) => {
              ...
              if(list.length>0){
                  for(let i=0;i<list.length;i++){
                      let data=list[i];
                        text+= `<div style='width:100%;border-bottom:${i === list.length-1 ? 0 : 1}px #e8eaec solid;height:30px;line-height:30px;'>`
                          text+="<p>"+data.excellentPercent+"% （"+data.excellentStudent+"人）</p></div>"
                  }
              }else{
                  text+="<p>"+row.excellentPercent+"% （"+row.excellentStudent+"人）</p>"
              }
              return h("div", {
                domProps: {
                    innerHTML: text
          ...
          render: (h, params) => {
              ...
              if(list.length>0){
                  for(let i=0;i<list.length;i++){
                      let data=list[i];
                        text+= `<div style='width:100%;border-bottom:${i === list.length-1 ? 0 : 1}px #e8eaec solid;height:30px;line-height:30px;'>`
                          text+="<p>"+data.goodPercent+"% （"+data.goodStudent+"人）</p></div>"
                  }
              }else{
                  text+="<p>"+row.goodPercent+"% （"+row.goodStudent+"人）</p>"
              }
              return h("div", {
                domProps: {
                    innerHTML: text
          ...
          render: (h, params) => {
              ...
              if(list.length>0){
                  for(let i=0;i<list.length;i++){
                      let data=list[i];
                        text+= `<div style='width:100%;border-bottom:${i === list.length-1 ? 0 : 1}px #e8eaec solid;height:30px;line-height:30px;'>`
                          text+="<p>"+data.passPercent+"% （"+data.passStudent+"人）</p></div>"
                  }
              }else{
                  text+="<p>"+row.passPercent+"% （"+row.passStudent+"人）</p>"
              }
              return h("div", {
                domProps: {
                    innerHTML: text
          ...
          render: (h, params) => {
            ...
            if(list.length>0){
              for(let i=0;i<list.length;i++){
                ...
                text+= `<div style='width:100%;border-bottom:${i === list.length-1 ? 0 : 1}px #e8eaec solid;height:30px;line-height:30px;'>`
                text+="<p>"+ count1 == 0 ? "0.00%" : `${data.finishStudent > 0 ? (count1 / data.finishStudent * 100).toFixed(2) : 0.00}% （${count1}人）</p></div>`
              }
            }else{
              text+="<p>"+ count == 0 ? "0.00%" : `${params.row.finishStudent > 0 ? (count && count / params.row.finishStudent * 100).toFixed(2) : 0.00}% （${count}人）</p>`
            }
            return h("div", {
              domProps: {
                innerHTML: text
        ...
        },
        {
          ...
          render: (h, params) => {
              ...
              let list=row.list;
              var text="";
              let count = row.excellentStudent + row.goodStudent + row.passStudent
              if(list.length>0){
                  for(let i=0;i<list.length;i++){
                      let data=list[i];
                      let count1 = data.excellentStudent + data.goodStudent + data.passStudent
                        text+= `<div style='width:100%;border-bottom:${i === list.length-1 ? 0 : 1}px #e8eaec solid;height:30px;line-height:30px;'>`
                          text+="<p>"+data.allPassPercent+"% （"+count1+"人）</p></div>"
                  }
              }else{
                  text+="<p>"+row.allPassPercent+"% （"+count+"人）</p>"
              }
              return h("div", {
                domProps: {
                    innerHTML: text
          ...
          series : [
              {
                  ...
                  center: ['50%', '60%'],
                  data:[
                      {value:exemptStudent, name:'免试率'},
                      {value:finishStudent, name:'完成率'},
                      {value:notJoinStudent, name:'未参加率'},
                      {value:notFinishStudent, name:'已参加未完成率'}
                  ],
                  itemStyle: {
                      emphasis: {
                          shadowBlur: 10,
          ...
          series : [
              {
                  ...
                  center: ['50%', '60%'],
                  data:[
                      {value:excellentNum , name:'优秀率',itemStyle:{}},
                      {value:goodNum , name:'良好率'},
                      {value:passNum , name:'及格率'},
                      {value:notPassNum, name:'不及格率'}
                  ],
                  itemStyle: {
                      emphasis: {
                          shadowBlur: 10,
           ...
           if(data.length>0){
               let examItemNum=data[0].list?data[0].list.length:1;
                //总数据
                    ...
                    totalData.excellentStudent = excellentStudent;
                    totalData.goodStudent = goodStudent;
                    totalData.passStudent = passStudent;
                ...
                if(examItemNum>1){
                    for(let j=0;j<examItemNum;j++){
                        ...
                        examItemData.excellentStudent = excellentStudent1;
                        examItemData.goodStudent = goodStudent1;
                        examItemData.passStudent = passStudent1;
