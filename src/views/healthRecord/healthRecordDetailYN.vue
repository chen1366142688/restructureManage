...
export default {
  ...
  methods: {
    ...
    queryStudentScoreByStudentId() {
      var vm = this;
      this.$axios.get("/v1/semesterScore/queryStudentScoreByStudentIdKM?studentId="+vm.studentId).then(res => {
        if (res.data.code == 10000) {
    ...
    getTestAgeResultStudent() {
        let vm = this;
        this.$axios.get("/v1/test/query/getTestAgeResultStudent?studentId="+vm.studentId).then(res => {
            if (res.data.code == 10000) {
                let ageResultData = res.data.response;
                /*是否完成体质测试 --待接口添加字段*/
                vm.isOkTest = ageResultData.testOverStatus == 1 ? true:false;
                if(!vm.isOkTest){return}
                ...
                for(let val of diagnosisData){
                    if(val.schoolItemId == 12&&val.teacherEditer == 1){
                        ...
                        break;
                         ...
                         vm.diagnosisContent = `本年度体质健康测试成绩为${ageResultData.currentTestScore}分，成绩优于${total}%的同龄人，年级排位${ageResultData.orderInfo?ageResultData.orderInfo.testOrder:'--'}。`
                            +`各测试项目分别反映了人体身体素质，其中`
                            +`${itemDesc}${deacItem}${descInfo}`;
                    }
                }
                let indicatorData = indicArr.filter(val => {return val.score;});
                    ...
                    for(let vals of ageResultData.standardList){
                        if(val.schoolItemId == vals.schoolTestId ){
                            ...
                            if(val.schoolItemId == 1){//BMI
                                ...
                                val.score = bmiScore;
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.bmiOrder : 0;
                                val.data = [];
                            }else if(val.schoolItemId == 2){//肺活量
                                val.resultValue = vitalCapacityValue;
                                val.score = vitalCapacityScore;
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.vitalCapacityOrder:0;
                                ...
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.runFiftyOrder:0;
                                ...
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.sitAndReachOrder:0;
                                val.data = vals.scoringCriteriaStart.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                            }else if(val.schoolItemId == 5){//一分钟跳绳值
                                val.resultValue = oneMinuteRopeSkippingValue;
                                val.score = oneMinuteRopeSkippingScore;
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.oneMinuteRopeSkippingOrder:0;
                                val.data = vals.scoringCriteriaStart.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                            }else if(val.schoolItemId == 6){//一分钟仰卧起坐
                                val.resultValue = oneMinuteSitUpValue;
                                val.score = oneMinuteSitUpScore;
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.oneMinuteSitUpOrder:0;
                                val.data = vals.scoringCriteriaStart.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                                ...
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.runFiftyEightOrder:0;
                                val.data = vals.scoringCriteriaStart.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                            }else if(val.schoolItemId == 8){//立定跳远
                                val.resultValue = standingJumpValue;
                                val.score = standingJumpScore;
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.standingJumpOrder:0;
                                val.data = vals.scoringCriteriaStart.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                            }else if(val.schoolItemId == 9){//引体向上
                                val.resultValue = pullUpsValue;
                                val.score = pullUpsScore;
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.pullUpsOrder:0;
                                val.data = vals.scoringCriteriaStart.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                                ...
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.runThousandOrder:0;
                                val.data = vals.scoringCriteriaEnd.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                                ...
                                val.gradeOrderCount = ageResultData.orderInfo?ageResultData.orderInfo.runEightHundredOrder:0;
                                val.data = vals.scoringCriteriaEnd.map((val)=>{return (val.resultValue/10<100&&val.resultValue/10>0)?(val.resultValue/10).toFixed(1):val.resultValue/10})
                            }
                        }
                    }
