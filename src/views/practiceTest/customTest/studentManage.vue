...
export default {
    ...
    return {
      ...
      dataColumns:[
        ...
        },
        {
          ...
          render:(h,parmas) => {
            let row = parmas.row;
            return h('div',row.testType == '1'?'统一测试':'学生自选')
          }
        },
        {
          title: "考核学生总数",
  ...
  methods: {
    ...
    studentManage(grade,id,name){//跳转学生管理详情
      this.$router.push({name:'customStudentitemmanage',query:{grade:grade,classTestId:id,activityId:this.$route.query.activityId,classTestName:name}});
    },
    back() {
      this.$router.go(-1);
    },
