...
export default {
  ...
  methods: {
    ...
    exportClassPlan(){
      ...
      let url =this.$axios.defaults.baseURL +
              "/v1/cd/high/sportsclass/exportCDHighClassPlan?token=" +
    ...
    exportSportsClassPlanErrorData() {
      var vm = this;
      let url =this.$axios.defaults.baseURL +
        "/v1/cd/high/sportsclass/exportSportsClassPlanErrorData?token=" +
        sessionStorage.getItem("token");
      window.open(url);
    },
  ...
  mounted() {
    ...
    vm.updateUrl =
      ...
      "/v1/cd/high/sportsclass/file/upload/sportsClassCDHighPlanExport?schoolId=" +
