<template>
  <div style="padding-left:10px;">
        ...
        <div @keydown.enter="querySearch" class="top-search">
            <div class="left-input">
                <div style="margin:5px 20px;">
                    <span>姓<span style="visibility:hidden">班级</span>名：</span>
    ...
    <Modal v-model="modal1" title="编辑得分" ok-text="保存" :mask-closable="false" @on-ok="saveStudentScore">
      <div style="padding-left:10px;">
        <div>
            <div style="margin-left:40px;">姓名：{{oneStudentName}}</div>
...
export default {
  ...
  data() {
    return {
      ...
      entryStatusList: [
        ...
        { entryStatus: 1, label: "已录入" },
        { entryStatus: 2, label: "未录入" },
      ...
      resultColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const text = row.entryStatus == "1" ? "已录入" : "未录入";
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const text =row.score != null? row.score/100:null;
            return h('div', ('1' != params.row.exemptHealth?text :'1' == params.row.exemptType?'70':'80'));
          }
        },
        {
          title: "操作",
            ...
            return h("div", [
              h(
                "Button",
                {
                  ...
                  attrs: {
                      disabled:params.row.exemptHealth == '1'?true:false
                  },
                  on: {
                    click: () => {
  ...
  methods: {
    ...
    handleSortChange ({column,key, order}) { // 传入的order是排序类型，升序降序默认
      ...
      vm.getClassStudentHealthyScoreList(sessionStorage.getItem("paperScoreManage-paperClassResults-classId"),
      sessionStorage.getItem("paperScoreManage-paperClassResults-paperId"));
    ...
    toDeleteResults(studentId){
      ...
      this.$axios
        .get(
          "/v1/healthyScore/updateStudentHealthyScoreStatus?studentId=" +studentId +"&paperId=" + vm.paperId
        )
    ...
    saveStudentScore() {
      ...
      this.$axios
        .get(
          "/v1/healthyScore/upsertStudentHealthyScore?paperId="+vm.paperId+"&studentId="+vm.oneStudentId+"&score="+vm.oneScore*100
        )
        .then(function(response) {
    ...
    getClassStudentHealthyScoreList(classId, paperId) {
      ...
      this.$axios
        .get(
          "/v1/healthyScore/getClassStudentHealthyScoreList?classId=" +
            classId +
            "&entryStatus=" +
            vm.entryStatus +
            "&paperId=" +
  ...
  mounted() {
    ...
    vm.gradeId = sessionStorage.getItem(
      "paperScoreManage-paperClassResults-gradeId"
    );
    vm.studentClass = sessionStorage.getItem(
      "paperScoreManage-paperClassResults-studentClass"
    );
    vm.classId = sessionStorage.getItem(
      "paperScoreManage-paperClassResults-classId"
    );
    vm.paperId = sessionStorage.getItem(
      "paperScoreManage-paperClassResults-paperId"
    );
    vm.getClassStudentHealthyScoreList(
      sessionStorage.getItem("paperScoreManage-paperClassResults-classId"),
      sessionStorage.getItem("paperScoreManage-paperClassResults-paperId")
    );
  }
};
