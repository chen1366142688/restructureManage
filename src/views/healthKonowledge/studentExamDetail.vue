<template>
  <div style="padding-left:10px;">
    ...
    <Card  style="margin-top:20px;" :dis-hover="true">
      <Form :label-width="100" class="function">
        <FormItem label="学生签名">
          ...
          <Button @click="clearSignPre" type="success" :disabled="signatureAddrTf">清除签名</Button>
          </div>
        </FormItem>
      </Form>
    </Card>
    ...
    <Modal v-model="updateModal" title="提醒" @on-ok="saveShowType" @on-cancel="cancel">
            <p>学生已签名，修改成绩后，将删除签名，学生需重新签名，是否确认修改？</p>
        </Modal>
        <Modal v-model="clearModal" title="清除签名" @on-ok="clearSign" @on-cancel="cancel">
            <p style="color:red">执行此操作后，将清除学生签名，是否确定开启学生重新签名？</p>
        </Modal>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      resultColumns: [
                ...
                ? [
                    h(
                      "Button",
                      {
                        ...
                        on: {
                          click: () => {
                            vm.querySchoolReportStatus().then(editStatus => {
                              if (editStatus == 1) {
                                if(vm.signatureAddr){
                                  vm.updateModal = true
                                }else{
                      ...
                      "Button",
                      {
                        ...
                        on: {
                          click: () => {
                            vm.querySchoolReportStatus().then(editStatus => {
                              if (editStatus == 1) {
                                vm.updateResult(params.row.examResultId)
                              } else {
                                vm.$Modal.info({
                                  title: '提醒',
                                  okText: '知道了',
        ...
        },
        {
          ...
          minWidth:100,
          render: (h, params) => {
            let { infoStatus } = params.row
            let text = infoStatus == 1 ? '有效' : '无效'
            return h('div', text)
          }
        },
        {
  ...
  methods: {
    // 查询教育局上报状态
    async querySchoolReportStatus() {
      let result = await this.$axios.get('/v1/subjectReport/querySchoolReportedStatus?dataType=2')
      if (result.data.code === 10000) {
        return result.data.response.editStatus
      }
    },
    updateStudentResultInfo(examResultId) {
      this.$axios.get(`/v1/studentexam/updateStudentTestDetailInfo?examResultId=${examResultId}`).then(res =>{
        if (res.data.code === 10000) {
          this.$Message.success('操作成功')
          this.getStudentTestDetailInfo(this.studentId,this.examItemId)
        }
      })
    },
    // 清除成绩
    updateResult(examResultId) {
      const that = this
      let {signatureAddrTf} = this
      let content = `${signatureAddrTf ? '' : '学生已签名，'}设为无效后，${signatureAddrTf ? '该项目成绩' : '成绩和签名都'}将被删除。`
      // 已签名
      this.$Modal.confirm({
        title: '提醒',
        okText: '设为无效',
        content: content,
        onOk: () => {
          that.updateStudentResultInfo(examResultId) // 清除学生测试成绩
        },
        onCancel: () => {
        }
      });
    ...
    clearSign() {
      ...
      this.$axios
        .get(
          "/v1/studentexam/clearSign?studentId=" +vm.studentId 
        )
        .then(function(res) {
          if (res.data.code == 10000) {
      ...
      this.$axios
        ...
        .then(res => {
          if (res.data.code == 10000) {
            ...
            vm.signatureAddrTf=data.signatureAddr?false:true
          } else {
